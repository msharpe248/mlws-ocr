...
from . import postpass, segment
from .seqterm import SeqTerms
...
from .segment import NUMERIC_PUNCT  # noqa: F401,E402  (defined with the line geometry that uses it)
...
from .postpass import DIGIT_TWINS  # noqa: F401,E402  (the digit mode and the post-passes share the twin table)
...
from .segment import gap_band  # noqa: F401,E402  (the band is defined with the rest of the line geometry)
...
@register
class BeamDecode(SeqTerms, Stage):
    slot = "decode"
    impl = "beam"
    defaults = {
        "beam_width": 8,
        ...
        "caps_page_all": False,   # ... and all-lower-case words of two or more letters too
                                  # ('tan woon yann' for a receipt's bold header)
    ...
    _low_mode = staticmethod(segment._low_mode)
    ...
    _line_x_height = staticmethod(segment._line_x_height)

    # First letters whose case twins pixels cannot separate: pure size
    # twins plus the tall pairs (i/I with its dot merged, b/B, p/P).
    _CASE_AMBIG = postpass._CASE_AMBIG
    _ABBREV = postpass._ABBREV

    # letter twins for digits misread inside words (reverse of DIGIT_TWINS)
    _DIGIT_TO_LETTER = postpass._DIGIT_TO_LETTER
    _NUM_SUFFIXES = postpass._NUM_SUFFIXES

    _is_bullet = staticmethod(postpass._is_bullet)

    _is_bar = staticmethod(postpass._is_bar)

    _mixed_alnum_repair = staticmethod(postpass._mixed_alnum_repair)

    _word_case_coherence = staticmethod(postpass._word_case_coherence)

    _dehyphenate_pass = staticmethod(postpass._dehyphenate_pass)

    _LOWER_SURE = postpass._LOWER_SURE
    _UPPER_SURE = postpass._UPPER_SURE

    def _line_case_pass(self, layout, p):
        return postpass._line_case_pass(layout, p)

    def _sentence_case_pass(self, layout, lm, p):
        return postpass._sentence_case_pass(layout, lm, p)
    ...
    def _shape_repairs(layout, p) -> dict:
        ...
        if p["caps_page_repair"]:
            ...
            out["caps_repairs"] = uppercase_caps_page([ln for ln in lines if not ln.get("graphic_suspect")],
                                                      all_lower=bool(p["caps_page_all"]))
        return out
    ...
    _segment_line = staticmethod(segment._segment_line)
    ...
    _merge_paths = staticmethod(segment._merge_paths)
